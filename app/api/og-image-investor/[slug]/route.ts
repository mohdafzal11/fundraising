...
export async function GET(request: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    const { backgroundImageName, investorName, width, height, fallbackImagePath } = options;
    ...
    let textMetrics = ctx.measureText(investorName);
    while (textMetrics.width > maxWidth && fontSize > 30) {
      fontSize -= 2;
      ctx.font = `bold ${fontSize}px Arial, sans-serif`;
      textMetrics = ctx.measureText(investorName);
    }
    ...
    ctx.fillText(investorName, textX, textY);
