...
export default function EditSectionForm({ initialData  , isLoading }: { initialData: Section | undefined, isLoading: boolean }) { 
  ...
  const [formData, setFormData] = useState<Section>({
    ...
    tableOfContent : initialData?.tableOfContent || "",
    description: initialData?.description || "",
    isActive: initialData?.isActive || true,
    isTableOfContentVisible: initialData?.isTableOfContentVisible ?? true,
 ...
 const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(getPageUrl(`/api/admin/sections/${formData.id}`), {
        method: "PUT",
        headers: {
              ...
              <div className="flex justify-between mt-8">
                ...
                {currentStep < steps.length - 1 ? (
                  ...
                  <Button 
                    ...
                    className="bg-primary text-primary-foreground hover:bg-primary/90"
                  >
                      {formIsSubmitting ? "Saving..." : "Update Section"}
                  </Button>
                )}
              </div>
            </form>
